...
A memorial site. Read `PLAN.md` before any structural change — it records what was
chosen, what was rejected, and why. `README.md` covers running and deploying.

Invariants, each with a reason. Check `PLAN.md` before overriding one:
...
- **Never render visitor-submitted text as HTML.** Guestbook messages are plain text,
  ...
  used for our own Markdown in `content/`. (§11)
- **`next/image` is for curated assets only.** Visitor-submitted photos are served
  ...
  stranger-supplied files to libvips on our own server is a real exploit path. (§12)
...
- **Fonts are committed `.woff2` files** loaded via `next/font/local`, never
  `next/font/google` — that loader fetches from Google at build time. (§11)
...
- **The database is temporary scaffolding.** The site freezes to static in year two,
  so avoid designs that assume Postgres is permanent. (§3, M6)
