# Joe Weisman Memorial — Revised Plan

> Supersedes `memorial-site-architecture.md`. That document is kept as the record of
> the original proposal; this one reflects decisions made after auditing it.
>
> **Timeline: service in September. Site live in about a week.** That's roughly six
> weeks of runway, which is enough to build the whole thing properly — the site is not
> a rush job, but the first week is. The plan is ordered so a real site is up in ~7 days
> and photo submissions open shortly after, well before the service.
